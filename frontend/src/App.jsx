...
async function getRealForecast(commodity, days = 30, market = "") {
  try {
...
function getMockPrice(commodity, days = 0){
  const base = BASE_PRICES[commodity] || 1500;
  const day = Number(days || 0);
  const seasonal = Math.sin(day / 10) * base * 0.07;
  const trend = base * (0.01 * Math.sin(day / 20) + 0.0008 * day);
  const random = Math.sin((day + commodity.length * 7) / 3) * base * 0.02;
  const price = Math.max(50, Math.round(base + seasonal + trend + random));
  return price;
}
...
function Login({onLogin,onBack}){
  ...
  const validate=()=>{
    ...
    if(tab==="farmer"&&form.mobile.trim()&&!/^\d{10}$/.test(form.mobile))e.mobile="10-digit mobile required";
        ...
        <div style={{...glass,padding:32}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <button onClick={()=>{
                ...
                const safeUser = {
                  name: form.name.trim() || (tab==="farmer"?"Farmer":"Trader"),
                  email: form.email.trim() || "user@example.com",
                  mobile: form.mobile.trim(),
                  location: form.location || NAGPUR_MARKETS[0],
                ...
                if(isValid){
                  ...
                  // still allow navigation for smoother dev/test flow
                  onLogin(safeUser);
                }
            }} style={{
...
useEffect(() => {
  getRealForecast(commodity, 30, market).then(data => {
    setForecast(data);
    if (data.length > 0) {
      setCur(data[0].price);
    }
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28}}>
          ...
          <div style={{display:"flex",gap:10}}>
            <select style={sel} value={market} onChange={e=>setMarket(e.target.value)}>
              {NAGPUR_MARKETS.map(m=><option key={m} value={m}>{m}</option>)}
            </select>
            <select style={sel} value={commodity} onChange={e=>setCommodity(e.target.value)}>
              {NAGPUR_COMMODITIES.map(c=><option key={c.name} value={c.name}>{c.emoji} {c.name}</option>)}
            </select>
          </div>
        </div>
        {/* Top 3 cards */}
...
useEffect(() => {
  getRealForecast(commodity, 30, market).then(setForecast);
}, [commodity, market]);
  const cur = forecast.length > 0 ? forecast[0].price : BASE_PRICES[commodity];
...
useEffect(() => {
  getRealForecast(commodity, 1, market).then(d => {
    if (d.length) setLivePrice(d[0].price);
  });
}, [commodity, market]);

useEffect(() => {
  getRealForecast(commodity, 7, market).then(setForecast7);
}, [commodity, market]);

  const cur = livePrice || BASE_PRICES[commodity] || 0;
  const p7 = forecast7[6]?.price || cur;
  const ch = cur > 0 ? ((p7 - cur) / cur * 100).toFixed(1) : 0;
  const sel={background:"rgba(255,255,255,0.04)",border:"1px solid rgba(74,222,128,0.15)",
...
export default function App(){
  ...
  if(page==="dashboard"){
    if(!user) return<Login onLogin={login} onBack={()=>nav("landing")}/>;
    return<Dashboard user={user} navigate={nav}/>;
  }
  if(page==="prediction"){
    if(!user) return<Login onLogin={login} onBack={()=>nav("landing")}/>;
    return<PricePrediction user={user} navigate={nav}/>;
  }
  if(page==="chat"){
    if(!user) return<Login onLogin={login} onBack={()=>nav("landing")}/>;
    return<AIChat user={user} navigate={nav}/>;
  }
